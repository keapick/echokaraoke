...
{
    bool retval = false;
    unsigned int frame = ms / CDG_FRAME_RATE;
    if ((ms % CDG_FRAME_RATE < 0) && (SkipFrames.size() > frame + 1) && (SkipFrames.at(frame + 1)))
        retval = true;
    if ((SkipFrames.size() > frame) && (SkipFrames.at(frame)))
        retval = true;
    return retval;
}
    ...
    while (CDGFileOpened)
    {
        if (!feof(CDGFile))
        {
            if (fread(&SubCode, sizeof(SubCode), 1, CDGFile) == 1)
            {
                ...
                CurPos++;
                if (((GetPosMS() % CDG_FRAME_RATE) == 0) && (GetPosMS() >= 40))
                {
                    CDG_Frame_Image *img;
                    if (needupdate)
                    {
                        LastCDGCommandMS = frame * CDG_FRAME_RATE;
                        lastupdate = frame;
                        needupdate = false;
...
{
    unsigned int frameno = ms / CDG_FRAME_RATE;
    if (ms % CDG_FRAME_RATE > 0) frameno++;
...
{
    unsigned int frameno = ms / CDG_FRAME_RATE;
    if ((ms % CDG_FRAME_RATE > 0) && (frameno + 1 < CDGVideo.size()) && (CDGVideo.at(frameno + 1)->RowChanged(row)))
        return true;
